...
<head>
    ...
    <script>
        $(function () {
            $("#name").text("管理员${name}您好");
            <c:forEach items="${goods}" var="good" varStatus="s">
            <c:if test="${good.name=='木材'}">
            $("#mucai").text("采购价：${good.purprice}元");
            </c:if>
            <c:if test="${good.name=='钢铁'}">
            $("#gangtie").text("采购价：${good.purprice}元");
            </c:if>
            <c:if test="${good.name=='玻璃'}">
            $("#boli").text("采购价：${good.purprice}元");
            </c:if>
            <c:if test="${good.name=='铝合金'}">
            $("#lvhejin").text("采购价：${good.purprice}元");
            </c:if>
            <c:if test="${good.name=='树脂'}">
            $("#shuz").text("采购价：${good.purprice}元");
            </c:if>
            <c:if test="${good.name=='防腐木'}">
            $("#fangfumv").text("采购价：${good.purprice}元");
            </c:if>
            <c:if test="${good.name=='混泥土'}">
            $("#hunnitu").text("采购价：${good.purprice}元");
            </c:if>
            <c:if test="${good.name=='胶水'}">
            $("#jiaoshui").text("采购价：${good.purprice}元");
            </c:if>
            <c:if test="${good.name=='塑料板'}">
            $("#lvsuoban").text("采购价：${good.purprice}元");
            </c:if>
            <c:if test="${good.name=='竹子纤维'}">
            $("#xianwei").text("采购价：${good.purprice}元");
            </c:if>
            ...
            $(".thumbnail").click(function () {
                ...
                if (num != null) {
                    ...
                    location.href = "/purServlet?id=" + goodsname + "&num=" + num + "&cid=1";
                }
            });
            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                    <ul class="nav navbar-nav">
                        <li class="active"><a href="/managergoodsServlet">采购商品 <span
